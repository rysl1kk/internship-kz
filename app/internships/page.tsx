...
export default function InternshipsPage() {
  ...
  const handleSaveChanges = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ username: editUsername, avatar_url: finalAvatarUrl })
      .eq("id", user.id);

    if (error) {
      alert("Этот никнейм уже занят или произошла ошибка!");
    } else {
      setUser((prev: any) => ({ ...prev, username: editUsername, avatar: finalAvatarUrl }));
  ...
  const handleEditAvatarSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) setUser((prev: any) => ({ ...prev, avatar: URL.createObjectURL(file) }));
  };
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      if (error) {
        ...
        if (file) finalAvatarUrl = await uploadAvatar(file, data.user.id);
        ...
        setUser({ id: data.user.id, email, username: registeredName, avatar: finalAvatarUrl });
      ...
      if (error) {
        ...
        setUser({ id: data.user.id, email: data.user.email, username: loggedInName, avatar: profile?.avatar_url || null });
  ...
  return (
    <div className="relative w-full overflow-hidden min-h-screen text-white bg-[#060b18]">
      {/* ЕДИНСТВЕННАЯ ЧИСТАЯ ШАПКА САЙТА */}
      <div className="w-full border-b border-slate-900/80 bg-[#060b18]/60 backdrop-blur-md sticky top-0 z-40">
        <header className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-12">
            <span className="font-black text-xl tracking-tight cursor-pointer" onClick={() => { setActiveTab("all"); setFilter("Все"); setSearchQuery(""); }}>
          ...
          {/* Правая часть: либо кнопка Войти, либо Профиль */}
          <div className="flex items-center">
            {user ? (
              <div 
                onClick={() => setIsProfileModalOpen(true)}
                className="flex items-center gap-3 bg-slate-900/60 border border-slate-800 rounded-2xl p-1.5 pr-4 hover:border-slate-600 transition cursor-pointer select-none"
              >
      ...
      {/* Анимированный фон (Сетка + Орбиты) */}
      ...
      <div className="mx-auto max-w-7xl px-6 py-20">
        ...
        {/* Внутренние вкладки переключения списков */}
      ...
      {/* Модальное окно: Подробнее о стажировке */}
      <AnimatePresence>
        {selectedJob && (
            ...
            >
              ...
              <div className="mt-8 pt-6 border-t border-slate-800/80 flex items-center justify-between">
                ...
                <button 
                  ...
                  className={`text-sm font-bold px-6 py-3.5 rounded-2xl border transition-all ${
                    appliedJobIds.includes(selectedJob.id)
                    ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-400"
                    : "bg-blue-600 border-blue-500 text-white hover:bg-blue-500"
                  }`}
                >
                  {appliedJobIds.includes(selectedJob.id) ? "✓ Вы откликнулись" : "Откликнуться"}
                </button>
              </div>
            </motion.div>
          </div>
      ...
      <AnimatePresence>
        {isProfileModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-sm p-8 rounded-3xl border border-slate-800 bg-slate-900/95 backdrop-blur-xl shadow-2xl z-10">
              <h2 className="text-xl font-black mb-1">Мой профиль</h2>
              <p className="text-slate-400 text-xs mb-6">Управление личными данными</p>
              <form onSubmit={handleSaveChanges} className="space-y-5">
                <div className="flex flex-col items-center gap-3 py-2">
                  <div onClick={() => editFileInputRef.current?.click()} className="w-20 h-20 rounded-2xl bg-slate-800 border-2 border-dashed border-slate-700 flex items-center justify-center cursor-pointer overflow-hidden group relative">
                    {user?.avatar ? <img src={user.avatar} alt="Avatar" className="w-full h-full object-cover" /> : <span className="text-slate-500 text-2xl">+</span>}
                  </div>
                ...
                <div>
                  ...
                  <input type="text" required value={editUsername} onChange={(e) => setEditUsername(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none" />
                </div>
                <div className="pt-2 space-y-2">
                  <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white font-bold p-3 rounded-xl text-sm">{loading ? "Сохранение..." : "Сохранить"}</button>
                  <button type="button" onClick={handleLogout} className="w-full bg-red-950/40 border border-red-900/40 text-red-400 font-bold p-3 rounded-xl text-sm">Выйти из аккаунта</button>
                </div>
              </form>
            </motion.div>
          </div>
      ...
      <AnimatePresence>
        {isAuthModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-md p-8 rounded-3xl border border-slate-800 bg-slate-900/90 backdrop-blur-xl shadow-2xl z-10">
              <h2 className="text-2xl font-black mb-6">{isSignUp ? "Регистрация" : "Вход в систему"}</h2>
              <form onSubmit={handleAuthSubmit} className="space-y-4">
                {isSignUp && (
                  <div>
                    <label className="block text-xs font-mono uppercase text-slate-500 mb-2">Никнейм</label>
                    <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="your_tag" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none" />
                  </div>
                )}
                <div>
                  <label className="block text-xs font-mono uppercase text-slate-500 mb-2">Email</label>
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@example.com" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-mono uppercase text-slate-500 mb-2">Пароль</label>
                  <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none" />
                </div>
                <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white font-bold p-3 rounded-xl text-sm transition disabled:opacity-50">
                  {loading ? "Загрузка..." : isSignUp ? "Зарегистрироваться" : "Войти"}
                </button>
              </form>
              <div className="mt-6 text-center">
                <button onClick={() => setIsSignUp(!isSignUp)} className="text-xs text-blue-400 hover:underline">
                  {isSignUp ? "Уже есть аккаунт? Войти" : "Нет аккаунта? Создать прямо сейчас"}
                </button>
              </div>
            </motion.div>
          </div>
